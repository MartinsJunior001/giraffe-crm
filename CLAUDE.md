...
**Estado transitório da implementação — Story 2.1 (ciclo de vida e catálogo de Pipes).** O L1 está fechado: existe banco com isolamento por RLS, autenticação e sessão (1.4/1.5), contexto de Organização resolvido no servidor (1.3), autorização por papel (1.6, CASL deny-by-default) e a casca navegável (1.7/1.8). O Épico 2 começou: **existe `Pipe`** — entidade organizacional com tabela, RLS, migration e API interna (criar, listar, obter, renomear, arquivar, restaurar).

**Não** existe ainda: papéis/acesso **por Pipe** (Story 2.2 — hoje só o ADMIN da Organização lê e administra Pipe; MEMBER e GUEST não têm acesso nenhum a Pipe), **Fases** (2.3), Formulários (2.4+), **Cards** (2.7+) nem Databases. A trava "não arquivar Pipe com Cards ativos" é **contrato futuro da 2.11** — não há tabela de Card, e nenhuma foi materializada para preparar o futuro (AD-11).
...
- `apps/api/src/pipes/` — primeiro domínio do Épico 2. `PipesService` faz **toda** query por `withTenantContext` — não há um único `where orgId` manual, porque um `where` se esquece e a policy não. `PipesController` expõe 6 rotas, todas com `@Requer`; nenhuma aceita `orgId` do cliente. **Não há rota de exclusão** — e isso não depende da rota: o runtime **não tem GRANT de DELETE** em `Pipe`. Arquivar é mudança de estado (`ACTIVE ⇄ ARCHIVED` + `archivedAt`), preserva os dados e é idempotente. `archive`/`restore` respondem **200** (transição, não criação); `POST /pipes` responde 201.
...
- Quem isola é o **banco**, não a aplicação: `Organization`, `Membership` e `Pipe` têm `ENABLE` **e** `FORCE ROW LEVEL SECURITY`. `Account` é global e **sem RLS** (AD-10) — a identidade não pertence a um tenant.
- **Toda tabela organizacional nova replica esse padrão** (a `Pipe` é o primeiro caso do Épico 2): policies `select/insert/update/delete` por `orgId = current_org_id()`, com `WITH CHECK` no INSERT **e** no UPDATE — sem o `WITH CHECK`, um INSERT com `orgId` alheio seria aceito e ficaria invisível, e um UPDATE poderia **mover** a linha para outra Organização.

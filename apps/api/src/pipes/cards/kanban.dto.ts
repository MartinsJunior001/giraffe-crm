...
/**
 * Validação de entrada MANUAL das rotas de leitura do Kanban (Story 2.9), no mesmo estilo dos demais DTOs do
 * domínio: aceita `unknown`, valida, devolve o tipo estreito — ou lança `BadRequestException` SANITIZADA. Sem
 * `class-validator` (Constitution II).
 */

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

/** Cursor de paginação = `id` do último Card da página anterior (UUID). Ausente → 1ª página. Lixo → 400. */

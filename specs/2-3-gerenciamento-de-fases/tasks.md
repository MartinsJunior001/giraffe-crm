...
- [ ] **T001** `schema.prisma`: enum `PhaseState` (ACTIVE/ARCHIVED); model `Phase`
...
- [ ] **T002** Migration `<ts>_phases/migration.sql` (ts > `pipe_grants`): enum + tabela + índice + FKs
...
- [ ] **T003** Rollback `<ts>_phases.down.sql` (DROP policies/tabela/enum), **sem tocar**
  `Pipe`/`PipeGrant`/`Membership`. `prisma generate`. [SC-239]
- [ ] **T004** `Phase` em `MODELOS_AUDITADOS` (`tenant-context.ts`).

## Phase 2: Autorização por recurso (serviço) — ATIVA o papel de Pipe
- [ ] **T005** `PhasesService.resolverPoderNoPipe(pipeId) → 'gerenciar' | 'ler'` (ou 404): Admin da Org →
...
- [ ] **T006** `PhasesService` (via `withTenantContext`): `listar` (ordem `position, id`; default ACTIVE),
...
- [ ] **T007** `PhasesController` sob `/pipes/:pipeId/phases` (+ DTOs): GET, POST, PATCH,
...
- [ ] **T008** `phases-rls.test.ts`: outra Org não vê; INSERT/SELECT/UPDATE sem contexto NEGADO (**fase
  vermelha**); dono ≠ runtime; **sem DELETE** (`permission denied`). [SC-238]
- [ ] **T009** `phases-http.test.ts`: criar/renomear na ordem; mover intra-Pipe não afeta outro Pipe;
...
- [ ] **T010** `phases-authz.test.ts` (**fecha DBT-2.2-ROLE-DORMENTE**): Admin da Org gerencia qualquer Pipe;
...
- [ ] **T011** Migration deploy (banco limpo) + rollback (sem tocar Pipe/PipeGrant/Membership) + reaplicação

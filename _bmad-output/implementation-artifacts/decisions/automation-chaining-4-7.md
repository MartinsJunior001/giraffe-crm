...
| **timeout por Ação** | `MAX_ACTION_DURATION_MS` | **30 s** | Uma mutação de domínio (tx) é rápida; 30 s é folga generosa antes de considerar a Ação lenta. Guarda lógico. |
| **timeout por Execução** | `MAX_EXECUTION_DURATION_MS` | **60 s** | Alinhado ao `LEASE_MS` da 4.6 (60 s): o lease já é o mecanismo FÍSICO de retomada (um `RUNNING` além do lease é reivindicado — recuperação de crash, 4.6); este é o guarda LÓGICO. |

...
- **Rule:** capacidade de arquivos condicional a Produto; metadados e autorização na aplicação/banco; buckets privados; **acesso por entrega autenticada sob a sessão do usuário (proxy/stream pela API)** [emenda 2026-07-17 — OQ-47/Opção A do dono; a redação original *"acesso por URL temporária e curta"* foi **substituída** por decisão registrada em `sprint-change-proposal-2026-07-17.md` e materializada na ADR-001; a entrega por proxy vincula ao usuário/recurso/finalidade e dispensa URL bearer]; validar tamanho/tipo/conteúdo, checksum, quarentena e verificação de arquivo malicioso; impedir acesso cruzado mesmo conhecendo a chave do objeto; ciclo de vida definido (upload incompleto, órfão, substituição, exclusão lógica/física, retenção, restauração/backup).
- **Materialização — DESIGN APROVADO E MERGEADO (PR #93, `d9f5ede`, 2026-07-17). NÃO IMPLEMENTAR ainda: a Story 3.7 não foi escrita.** Ver `docs/03-arquitetura/adr-001-capacidade-de-arquivos.md` (ADR-001, status **RATIFICADA — design**). **A ADR EMENDA esta Rule num ponto; instancia o resto** — o dono escolheu **entrega por proxy sob sessão (Opção A)**. As duas divergências foram **ratificadas** pelo workflow oficial (`sprint-change-proposal-2026-07-17.md`, `bmad-correct-course`): DIV-1 (esta Rule) e DIV-2 (epics 3.7 AC#2) **resolvidas**. O design passou por **quatro revisões read-only independentes** (v2/v3 reprovadas; v4 aprovada com ressalvas; v5 aprovada). **A 3.7 está DESBLOQUEADA no nível de design** e elegível para o fluxo BMAD (`bmad-create-story` → Spec Kit); os **gates de runtime** (`pre-implementation-check`, `security-check`, `migration-check`, `lgpd-check`) permanecem obrigatórios **no momento de escrever o código** — a implementação é passo futuro **ainda não autorizado**. Instanciados pela ADR: **10 MiB/arquivo** e **10 por RECURSO** (`FILE_MAX_PER_RESOURCE`, **contagem** — decisão Q1 do dono; **não** "por Registro": desacoplada de Card/Registro); **rate limit + semáforo de scan pertencem à 3.7**, com a **extração do primitivo antiabuso como tech story pré-requisito** (decisão Q2 do dono; a 3.8 só acrescenta limites do canal público); **`.txt/.csv/.json` fora da allowlist inicial** sem enfraquecer o gate de magic bytes (decisão Q3 do dono); **ClamAV com `AlertExceedsMax`** + **canário EICAR e frescor da base**; chave **opaca** `<orgId>/<uuid>` com guarda de prefixo **por segmento**; **dois buckets** (quarentena × liberados); **magic bytes + allowlist enumerada, sem SVG**; `attachment` + `Content-Type` detectado + `nosniff` + CSP + `no-store`; **duas tabelas — `FileObject` (ciclo de vida, mutável) e `FileScan` (fato apurado, append-only)** — ambas **RLS ENABLE+FORCE + WITH CHECK e GRANT sem DELETE**, o hash forense imutável **pelo banco** em `FileScan`. **Notas anteriores desta linha descreviam a v1 (URL assinada ≤ 60 s, "10 por Registro") e a v2 — ambas revogadas.** Histórico: **v2/v3/v4 passaram por revisões independentes** (v2 e v3 reprovadas; v4 aprovada com ressalvas, fechadas na v5).
...
- **Estado do gate:** **arquivos — design APROVADO e mergeado (ADR-001, PR #93); a capacidade segue fail-closed em runtime.** OQ-47 decidida, Opção A escolhida, limites Q1/Q2/Q3 fixados, divergências DIV-1/DIV-2 emendadas pelos workflows oficiais, e o design ratificado por 4 revisões independentes. **O gate de RUNTIME NÃO muda com o merge do design:** `FILE_UPLOAD_ENABLED` segue default `false` (`env.ts:169`), e ausência/ilegibilidade de qualquer limite ou do scanner **nega** — destravar em runtime é ato da implementação da 3.7 (com `security-check` no código), não deste merge documental. **E-mail outbound e IA seguem GATED** (OQ-32, OQ-43..46 abertas) — o destravamento seria por capacidade, não do AD-28 inteiro.

...
- **Estado do gate:** **arquivos — decisão de Produto (OQ-47) tomada e divergências autoritativas RESOLVIDAS; falta só o merge da ADR + `security-check`.** O dono decidiu a OQ-47 e a Opção A; a ADR-001 fixou os limites numéricos e as decisões Q1/Q2/Q3; e as duas divergências (DIV-1: AD-27; DIV-2: epics 3.7 AC#2) foram **emendadas pelos workflows oficiais** (`sprint-change-proposal-2026-07-17.md`). **Destravar exige agora apenas: merge da ADR v5 + `security-check`.** O fail-closed **permanece em runtime** de todo modo: `FILE_UPLOAD_ENABLED` default `false` (`env.ts:169`), e ausência/ilegibilidade de qualquer limite ou do scanner **nega**. **E-mail outbound e IA seguem GATED** (OQ-32, OQ-43..46 abertas) — o destravamento seria por capacidade, não do AD-28 inteiro.

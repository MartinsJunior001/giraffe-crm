...
- **Materialização proposta — EM REVISÃO. NÃO IMPLEMENTAR AINDA.** Ver `docs/03-arquitetura/adr-001-capacidade-de-arquivos.md` (ADR-001, status **PROPOSTA**). **A ADR EMENDA esta Rule num ponto; instancia o resto** — o dono escolheu **entrega por proxy sob sessão (Opção A)**, logo **não há "URL temporária e curta"**: essa cláusula fica **suspensa para arquivos** até o **workflow oficial de Arquitetura** ratificar a emenda (DIV-1 da ADR). **A epics 3.7 AC#2 exige a mesma cláusula e também precisa de emenda pelo workflow BMAD** (DIV-2). **Enquanto as duas emendas não forem ratificadas, a Story 3.7 não entra em `ready-for-dev`.** Instanciados pela ADR: **10 MiB/arquivo** e **10 por RECURSO** (`FILE_MAX_PER_RESOURCE` — **não** "por Registro": a epics 3.7 exige a capacidade **desacoplada de Card/Registro**); **ClamAV com `AlertExceedsMax`** + **canário EICAR e frescor da base** (sem eles o clamd responde `OK` ao estourar os próprios limites **ou com base vazia**, e o malware é promovido como limpo); chave **opaca** `<orgId>/<uuid>` com guarda de prefixo **por segmento**; **dois buckets** (quarentena × liberados); **magic bytes + allowlist enumerada, sem SVG**; `attachment` + `Content-Type` detectado + `nosniff` + CSP + `no-store`; **RLS ENABLE+FORCE + WITH CHECK e GRANT sem DELETE** em `FileObject`. **Notas anteriores desta linha descreviam a v1 (URL assinada ≤ 60 s, "10 por Registro") e a v2 — ambas revogadas.** Histórico: **v2 e v3 foram REPROVADAS por duas revisões independentes**; a v4 fecha os achados e mantém abertas as duas divergências acima e três decisões do dono (Q1 limite por recurso em bytes × contagem; Q2 escopo do rate limit, que a `epics.md:1193` põe na 3.8; Q3 allowlist de tipos sem magic bytes).
...
- **Estado do gate:** **arquivos — a DECISÃO de Produto (OQ-47) existe; o gate NÃO está destravado.** O dono decidiu a OQ-47 e a Opção A, e a ADR-001 fixou os limites numéricos que a epics exigia **antes** das Stories de upload. Mas a ADR está em **PROPOSTA**, com **duas divergências autoritativas abertas** (DIV-1: AD-27 "URL temporária e curta"; DIV-2: epics 3.7 AC#2) que só os workflows oficiais de Arquitetura e BMAD podem ratificar. **Destravar exige as duas emendas + `security-check`.** *(Uma nota anterior desta linha dizia "DESTRAVADO" na mesma página em que o AD-27 dizia "NÃO IMPLEMENTAR AINDA" — contradição corrigida aqui.)* O fail-closed **permanece em runtime** de todo modo: `FILE_UPLOAD_ENABLED` default `false` (`env.ts:169`), e ausência/ilegibilidade de qualquer limite ou do scanner **nega**. **E-mail outbound e IA seguem GATED** (OQ-32, OQ-43..46 abertas) — o destravamento seria por capacidade, não do AD-28 inteiro.
